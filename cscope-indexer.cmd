...
:GET_RECURSE_SRC
dir /S /B | grep -i -v -E "\.svn|\.pdb|\.lib|\.dll|\.idb|\.ncb" > %LIST_FILE%.tmp
goto CSCOPE
...
:ALMOST_DONE
echo grep -i -E "\.([chly](xx|pp)*|cc|hh)$" %LIST_FILE%.tmp %LIST_FILE%
grep -i -E "\.([chly](xx|pp)*|cc|hh)$" %LIST_FILE%.tmp > "%LIST_FILE%"
goto DONE

:DONE
sed -e "s/\\/\//g" "%LIST_FILE%.tmp" > "%LIST_FILE%.anything.filelist"
del "%LIST_FILE%.tmp"
echo Done
