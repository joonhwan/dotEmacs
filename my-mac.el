...
;;  initial-major-mode 'emacs-lisp-mode ;; *scratch* shows up in
;; 									 ;; emacs-lisp-mode
;; 									 ;; aquamacs-default-major-mode
;; 									 ;; 'emacs-lisp-mode ; new buffers
;; 									 ;; open in emacs-lisp-mode
;;  )

;; ;; Frame and window management:
(if (featurep 'tabbar-mode)
    (tabbar-mode -1))		     ;; no tabbar
;; (one-buffer-one-frame-mode -1)       ;; no one-buffer-per-frame
;; (setq special-display-regexps nil)   ;; do not open certain buffers in
;; 									 ;; special windows/frames

;; (scroll-bar-mode -1)  ; no scrollbars

;; ;; Appearance
(if (functionp 'aquamacs-version)
    (progn
