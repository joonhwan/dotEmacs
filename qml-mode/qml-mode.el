...
(defvar qml-mode-viewer-command-format "qmlviewer %s"
  "QML viewer command line program.")

(defvar qml-mode-save-before-view t
  "Save current QML buffer before viewing, if set t")
...
(defconst qml-font-lock-keywords
  (let ((kw1 (mapconcat 'identity
                        '("Qt" "import" "property"
                          ...
                          "QtObject" "WorkerScript" "Item" "Rectangle" "Gradient"
...
(defun qml-insert-and-indent (arg)
  (interactive "*P")
...
;;
;; hacked from `graphviz-dot-view'
;;
(defun qml-run-qmlviewer(&optional arg)
...
buffer is saved before the command is executed."
  (interactive "P")
  (let ((cmd (if arg
				 (read-from-minibuffer "qml-view command: " 
									   (format qml-mode-viewer-command-format
											   (buffer-file-name)))
               (format qml-mode-viewer-command-format (buffer-file-name)))))
    (if qml-mode-save-before-view 
        (save-buffer))
...
(define-derived-mode qml-mode js-mode "QML"
  "Major mode for QML - Qt Markup Language"
...
;; (define-global-minor-mode qml-auto-insert-mode auto-insert-mode
;;   (lambda ()
;;     (when (not (memq major-mode
;;                      (list 'qml-mode)))
;;       (auto-insert-mode))))
;; (qml-auto-insert-mode 1)

(add-hook 'qml-mode-hook 'my-qml-mode-hook)

(define-key qml-mode-map "}" 'qml-insert-and-indent)
(define-key qml-mode-map (kbd "C-c RET") 'qml-run-qmlviewer)
(define-key qml-mode-map (kbd "<f5>") 'qml-run-qmlviewer)
